      ...
      <main dir={dir} className="min-h-screen bg-slate-50">
        <div className="mx-auto max-w-7xl px-4 py-8">
          <div className="mb-6 flex items-center justify-between">
            ...
            <InstantLink href="/invest/calculator" className="rounded-xl border px-4 py-2 hover:bg-slate-50">{t("invest.calculator","محاكي العوائد")}</InstantLink>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {items.map(op => (
              <InstantLink key={op.id} href={`/invest/${op.id}`} className="block rounded-2xl border bg-white p-4 shadow-sm hover:shadow-md transition">
