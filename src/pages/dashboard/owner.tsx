...
const OwnerDashboard: NextPage = () => {
  ...
  const ownerBookings = useMemo(() => {
    ...
    // ? ãÄŞÊÇğ: ÅĞÇ áã äÌÏ Ãí ÍÌæÒÇÊ¡ äÚÑÖ Çáßá ááÇÎÊÈÇÑ
  ...
  return (
    <>
      <Head>
        <title>áæÍÉ ÊÍßã ÇáãÚáä | Ain Oman</title>
      </Head>

      <div className="min-h-screen bg-gray-50">
        <header className="bg-white shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <h1 className="text-2xl font-bold text-gray-900">áæÍÉ ÊÍßã ÇáãÚáä</h1>
            <p className="text-gray-600">ãÑÍÈÇğ {text(session?.user?.name)}</p>
          </div>
        </header>

        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* äÙÑÉ ÚÇãÉ Úáì ÇáÅÍÕÇÆíÇÊ */}
          ...
          <div className="mt-8 border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              {[
                { id: "properties", name: "ÚŞÇÑÇÊí", count: properties.length },
                { id: "rentals", name: "ØáÈÇÊ ÇáÅíÌÇÑ", count: rentals.length },
                { id: "analytics", name: "ÇáÊŞÇÑíÑ", count: 0 }
              ].map((tab) => (
                <button
                  key={tab.id}
              ...
              >
                ...
                {properties.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">áÇ ÊæÌÏ ÚŞÇÑÇÊ</h3>
                    <p className="mt-1 text-sm text-gray-500">ÇÈÏÃ ÈÅÖÇİÉ ÚŞÇÑß ÇáÃæá.</p>
              ...
              >
                ...
                {rentals.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">áÇ ÊæÌÏ ØáÈÇÊ ÅíÌÇÑ</h3>
                    <p className="mt-1 text-sm text-gray-500">ÚäÏãÇ íÍÌÒ ÃÍÏ ÇáÚãáÇÁ ÚŞÇÑß¡ ÓÊÙåÑ ÇáØáÈÇÊ åäÇ.</p>
                  </div>
                )}
              </motion.div>
            )}
...
function getStateLabel(state: string): string {
  const states: Record<string, string> = {
    "reserved": "Êã ÇáÍÌÒ",
    "paid": "Êã ÇáÏİÚ",
    "docs_submitted": "Êã ÑİÚ ÇáãÓÊäÏÇÊ",
    "docs_verified": "Êã ÇáÊÍŞŞ",
    "contract_generated": "Êã ÊæáíÏ ÇáÚŞÏ",
    "tenant_signed": "Êã ÊæŞíÚ ÇáãÓÊÃÌÑ",
    "owner_signed": "Êã ÊæŞíÚ ÇáãÇáß",
    "accountant_checked": "Êã ÇÚÊãÇÏ ÇáãÍÇÓÈ",
    "admin_approved": "ÊãÊ ÇáãæÇİŞÉ ÇáÅÏÇÑíÉ",
    "handover_ready": "ÌÇåÒ ááÊÓáíã",
    "handover_completed": "Êã ÇáÊÓáíã"
  };
