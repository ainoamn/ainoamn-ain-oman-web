        ...
        {loading ? <div>جارٍ التحميل…</div> : (
          <>
                ...
                <tbody>
                  {items.sort((a,b)=>a.order-b.order).map((it, idx) => (
                    <tr key={it.id} className="border-t">
                      ...
                      <td className="p-2"><input className="border rounded p-1 w-40" value={it.id} onChange={(e)=>update(idx,{id:e.target.value})} /></td>
                      <td className="p-2"><input className="border rounded p-1 w-40" value={it.label} onChange={(e)=>update(idx,{label:e.target.value})} /></td>
                      <td className="p-2">
                        <select className="border rounded p-1" value={it.action} onChange={(e)=>update(idx,{action:e.target.value as ActionId})}>
                          {ACTION_OPTIONS.map(op => <option key={op} value={op}>{op}</option>)}
                        </select>
                      </td>
                      <td className="p-2"><input className="border rounded p-1 w-56" value={it.href ?? ""} onChange={(e)=>update(idx,{href:e.target.value})} placeholder="https://..." /></td>
                      <td className="p-2 w-24"><input type="number" className="border rounded p-1 w-20" value={it.order} onChange={(e)=>update(idx,{order: e.target.value ? +e.target.value : it.order})} /></td>
                      <td className="p-2 text-center"><input type="checkbox" checked={it.visible} onChange={(e)=>update(idx,{visible:e.target.checked})} /></td>
                      <td className="p-2"><button className="text-red-600 underline" onClick={()=>remove(idx)}>حذف</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex items-center gap-2 mt-3">
              <button onClick={addRow} className="px-3 py-2 rounded bg-gray-200">إضافة زر</button>
              <button onClick={save} disabled={saving} className="px-4 py-2 rounded bg-[var(--brand-800)] hover:bg-[var(--brand-700)] text-white">
                {saving ? "جارٍ الحفظ…" : "حفظ التغييرات"}
              </button>
              <button onClick={reset} className="px-3 py-2 rounded bg-rose-100 text-rose-700">استرجاع الافتراضي</button>
            </div>
          </>
        )}
      </div>
