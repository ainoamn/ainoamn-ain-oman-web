...
function resolveSrc(name?:string){
  ...
  // إذا كان الاسم يبدأ بـ uploads أو public/uploads
  return `/uploads/${name}`;
}
...
export default function AdminPropertiesList(){
  ...
  // تبديل حالة النشر للوحدة حسب unitNo
  ...
  async function archiveBuilding(id:string){
    if(!confirm("هل تريد أرشفة المبنى؟")) return;
    const r = await fetch(`/api/buildings/${encodeURIComponent(id)}`, {
      method:"PATCH", headers:{ "content-type":"application/json" },
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="container mx-auto p-4 flex-1 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <InstantLink href="/properties/new" className="btn btn-primary">عقار جديد</InstantLink>
        </div>
          ...
          <div className="overflow-auto">
            <table className="w-full text-sm border">
              ...
              <tbody>
                    ...
                    <td className="p-2">
                      {b.units.map(u=>(
                        <div key={`${b.id}-${u.unitNo}`} className="flex items-center justify-between gap-2 border-b last:border-b-0 py-1">
                          ...
                          <div className="flex items-center gap-2">
                            <InstantLink className="btn btn-outline btn-sm" href={`/admin/rent/${encodeURIComponent(b.id)}/${encodeURIComponent(u.id)}`}>إدارة الوحدة</InstantLink>
                ...
                {filtered.length===0 && <tr><td className="p-3 text-center text-gray-600" colSpan={4}>لا يوجد بيانات.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
