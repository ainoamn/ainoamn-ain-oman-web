...
export default function AdminContractsList(){
  ...
  useEffect(()=>{ setLoading(true); setErr(null);
    fetch("/api/contracts").then(r=>r.json()).then(d=>setItems(Array.isArray(d?.items)?d.items:[])).catch(()=>setErr("فشل التحميل")).finally(()=>setLoading(false));
  },[]);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="container mx-auto p-4 flex-1 space-y-4">
        ...
        <input className="form-input w-full max-w-sm" placeholder="ابحث بالرقم أو اسم المستأجر" value={q} onChange={e=>setQ(e.target.value)} />
              ...
              <tbody>
                {filtered.map(c=>{
                  ...
                  const badge = typeof days==="number" ? (
                    <span className={`px-2 py-1 rounded text-xs ${days<=0?"bg-red-100 text-red-800":days<=30?"bg-yellow-100 text-yellow-800":"bg-green-100 text-green-800"}`}>{days<=0?"منتهي":`متبقي ${days} يوم`}</span>
                  ) : "-";
                  return (
                    <tr key={c.id} className="border-b">
                      <td className="p-2">{c.contractNumber}</td>
                      <td className="p-2">{c.scope==="unified"?"موحد":"لكل وحدة"}</td>
                      ...
                      <td className="p-2"><InstantLink href={`/admin/contracts/${encodeURIComponent(c.id)}`} className="btn btn-outline">عرض</InstantLink></td>
                    </tr>
                  );
                })}
