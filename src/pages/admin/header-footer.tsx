...
const defaultHeader: HeaderSettings = {
  ...
  menuItems: [{ label: "«·—∆Ì”Ì…", href: "/" }, { label: "«·⁄ﬁ«—« ", href: "/properties" }, { label: "«·„“«œ« ", href: "/auctions" }],
...
const defaultFooter: FooterSettings = {
  ...
  sections: [
    { title: "⁄‰ «·„‰’…", links: [{ label: "„‰ ‰Õ‰", href: "/about" }, { label: " Ê«’· „⁄‰«", href: "/contact" }] },
    { title: "Œœ„« ", links: [{ label: "«·⁄ﬁ«—« ", href: "/properties" }, { label: "«·„“«œ« ", href: "/auctions" }] },
  ],
  contact: { email: "info@example.com", phone: "+96800000000", address: "„”ﬁÿ° ⁄ı„«‰" },
...
export default function HeaderFooterAdminPage() {
  ...
  if (!ready) return <div style={{ padding:24 }}>Ã«—ˆ «· Õ„Ì·Ö</div>;
  ...
  return (
    <>
      <Head><title>·ÊÕ… «·ÂÌœ— Ê«·›Ê — | Ain Oman</title></Head>
      ...
      <header style={stickyHeaderStyle}>
        <div style={{ maxWidth:1120, margin:"0 auto", padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <strong style={{ opacity:.9 }}>⁄Ì‰ ⁄ı„«‰</strong>
          </div>
      ...
      <main style={{ maxWidth:1120, margin:"16px auto 24px", padding:"0 16px" }}>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
          <button onClick={()=>setTab("header")} style={tabBtn(tab==="header")}>«·ÂÌœ—</button>
          <button onClick={()=>setTab("footer")} style={tabBtn(tab==="footer")}>«·›Ê —</button>
        </div>

        {tab==="header" ? (
          <div style={{ display:"grid", gap:16, gridTemplateColumns:"1fr 1fr" }}>
            <section style={panel}>
              <h2 style={h2}>√·Ê«‰ «·ÂÌœ—</h2>
              <Row label="·Ê‰ «·Œ·›Ì…">
              ...
              <Row label="≈ŸÂ«— ›Ì «·ÂÌœ—">
                <label style={{ display:"inline-flex", alignItems:"center", gap:8 }}>
                  ...
                  <span style={{ fontSize:14 }}>„ı»œ¯· «··Ê‰</span>
                </label>
              </Row>
                ...
                {header.availableColors.map((c,i)=>(
                  <div key={i} style={{ display:"flex", gap:8, alignItems:"center" }}>
                    ...
                    <button onClick={()=> saveHeader({ availableColors: header.availableColors.filter((_,x)=>x!==i) })}
                      style={btnDanger}>Õ–›</button>
                  </div>
                ))}
                <div>
                  <button onClick={()=> header.availableColors.length<5 && saveHeader({ availableColors:[...header.availableColors, "#0ea5e9"] })}
                    style={btnPrimary} disabled={header.availableColors.length>=5}>+ ·Ê‰</button>
                </div>
              </div>
            </section>

            <section style={panel}>
              <h2 style={h2}>ﬁÊ«∆„ «·ÂÌœ—</h2>
              {header.menuItems.map((m,i)=>(
                <div key={i} style={row}>
                  ...
                  <button style={btnDanger} onClick={()=> saveHeader({ menuItems: header.menuItems.filter((_,x)=>x!==i) })}>Õ–›</button>
                </div>
              ))}
              <div style={row}>
                <button style={btnPrimary} onClick={()=> saveHeader({ menuItems: [...header.menuItems, { label:"—«»ÿ ÃœÌœ", href:"#"}] })}>+ ⁄‰’—</button>
              </div>
          ...
          <div style={{ display:"grid", gap:16, gridTemplateColumns:"1fr 1fr" }}>
            ...
            <section style={panel}>
              <h2 style={h2}>„⁄·Ê„«  «· Ê«’·</h2>
              <Row label="«·»—Ìœ"><Input value={footer.contact.email} onChange={(e)=>saveFooter({ contact:{ ...footer.contact, email:e.target.value }})}/></Row>
              <Row label="«·Â« ›"><Input value={footer.contact.phone} onChange={(e)=>saveFooter({ contact:{ ...footer.contact, phone:e.target.value }})}/></Row>
              <Row label="«·⁄‰Ê«‰"><Input value={footer.contact.address} onChange={(e)=>saveFooter({ contact:{ ...footer.contact, address:e.target.value }})}/></Row>
            </section>
              ...
              {footer.sections.map((s,i)=>(
                <div key={i} style={{ border:"1px solid #e5e7eb", borderRadius:10, padding:12, marginBottom:10 }}>
                  <Row label={`«·⁄‰Ê«‰ #${i+1}`}>
                    ...
                    <button style={btnDanger} onClick={()=> saveFooter({ sections: footer.sections.filter((_,x)=>x!==i) })}>Õ–› «·ﬁ”„</button>
                  </Row>
                  ...
                  <div style={row}>
                    <button style={btnPrimary} onClick={()=> {
                      const arr=[...footer.sections]; arr[i]={ ...s, links:[...s.links, { label:"—«»ÿ", href:"#"}] }; saveFooter({ sections: arr });
                    }}>+ —«»ÿ</button>
                  </div>
                </div>
              ))}
              <div style={row}>
                <button style={btnPrimary} onClick={()=> saveFooter({ sections:[...footer.sections, { title:"ﬁ”„ ÃœÌœ", links:[{ label:"—«»ÿ", href:"#"}] } ] })}>+ ﬁ”„</button>
              </div>
            </section>

            <section style={{ ...panel, gridColumn:"1 / -1" }}>
              <h2 style={h2}>ÿ—ﬁ «·œ›⁄</h2>
              {footer.payments.map((p,i)=>(
                <div key={i} style={row}>
                  ...
                  }} placeholder="—«»ÿ «·√ÌﬁÊ‰… («Œ Ì«—Ì)"/>
                  <button style={btnDanger} onClick={()=> saveFooter({ payments: footer.payments.filter((_,x)=>x!==i) })}>Õ–›</button>
                </div>
              ))}
              <div style={row}>
                <button style={btnPrimary} onClick={()=> saveFooter({ payments:[...footer.payments, { name:"ÿ—Ìﬁ… ÃœÌœ…" }] })}>+ ÿ—Ìﬁ…</button>
              </div>
            </section>
          </div>
        )}
      ...
      <footer style={{ background: footerBg, color: footer.textColor, borderTop:"1px solid rgba(255,255,255,0.2)" }}>
        <div style={{ maxWidth:1120, margin:"0 auto", padding:"32px 16px" }}>
          <div style={{ display:"grid", gap:24, gridTemplateColumns:"repeat(4,minmax(0,1fr))" }}>
            <div><div style={{ fontWeight:600, fontSize:18 }}>⁄Ì‰ ⁄ı„«‰</div><p style={{ opacity:.9, marginTop:8, fontSize:14 }}>„‰’¯… ⁄ﬁ«—Ì… ·≈œ«—… «·⁄ﬁ«—«  Ê«·„“«œ« .</p></div>
            {footer.sections.map((s,i)=>(
              <div key={i}>
            ...
            <div>
              <div style={{ fontWeight:600 }}> Ê«’·</div>
              <ul style={{ marginTop:12, fontSize:14, lineHeight:"28px" }}>
                <li>«·»—Ìœ: <a href={`mailto:${footer.contact.email}`} style={{ color:"inherit" }}>{footer.contact.email}</a></li>
                <li>«·Â« ›: <a href={`tel:${footer.contact.phone}`} style={{ color:"inherit" }}>{footer.contact.phone}</a></li>
                <li>«·⁄‰Ê«‰: {footer.contact.address}</li>
              </ul>
              <div style={{ marginTop:16, fontSize:12, opacity:.9 }}>ÿ—ﬁ «·œ›⁄: {footer.payments.map(p=>p.name).join(" ï ")}</div>
            </div>
          </div>
          <div style={{ marginTop:24, paddingTop:16, borderTop:"1px solid rgba(255,255,255,0.2)", fontSize:12, opacity:.8, display:"flex", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
            <div>© {new Date().getFullYear()} ⁄Ì‰ ⁄ı„«‰</div>
            <div style={{ display:"flex", gap:16 }}>
              <a href="/terms" style={{ color:"inherit" }}>«·‘—Êÿ</a>
              <a href="/privacy" style={{ color:"inherit" }}>«·Œ’Ê’Ì…</a>
            </div>
          </div>
        </div>
      </footer>
