// src/pages/booking/new.tsx
// ’›Õ… ÕÃ“ «·⁄ﬁ«— - „Õ”‰… ··√œ«¡ «·›«∆ﬁ ?

import { useState, useEffect } from 'react';
import InstantImage from '@/components/InstantImage';
import { useRouter } from 'next/router';
import Head from 'next/head';
import InstantLink from '@/components/InstantLink';
import { toSafeText } from '@/components/SafeText';
import { useBookings } from '@/context/BookingsContext';
import { 
  FaCalendarAlt, FaUser, FaPhone, FaEnvelope, FaCreditCard,
  FaCheckCircle, FaInfoCircle, FaArrowRight, FaArrowLeft
} from 'react-icons/fa';

interface Property {
  id: string;
  title: any; // ﬁœ ÌﬂÊ‰ string √Ê {ar, en}
  priceOMR: number;
  type: any; // ﬁœ ÌﬂÊ‰ string √Ê {ar, en}
  images?: string[];
}

export default function NewBookingPage() {
  const router = useRouter();
  const { propertyId } = router.query;
  const { addBooking } = useBookings(); // ? «” Œœ«„ Context ·· ÕœÌÀ «·›Ê—Ì
  
  const [property, setProperty] = useState<Property | null>(null);
  const [loading, setLoading] = useState(true);
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    // »Ì«‰«  «·„” √Ã—
    name: '',
    email: '',
    phone: '',
    idNumber: '',
    
    // »Ì«‰«  «·ÕÃ“
    startDate: '',
    duration: '12', // »«·√‘Â—
    paymentMethod: 'bank_transfer',
    
    // „·«ÕŸ« 
    notes: '',
    
    // „Ê«›ﬁ« 
    agreedToTerms: false,
  });
  
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    if (propertyId) {
      loadProperty();
    }
  }, [propertyId]);

  const loadProperty = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/properties/${propertyId}`);
      if (response.ok) {
        const data = await response.json();
        setProperty(data.item || data);
      }
    } catch (error) {

    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const response = await fetch('/api/bookings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          unitId: propertyId, // «” Œœ«„ propertyId ﬂ‹ unitId
          buildingId: propertyId, // ·· Ê«›ﬁ
          startDate: formData.startDate,
          durationMonths: parseInt(formData.duration),
          paymentMethod: formData.paymentMethod,
          tenant: {
            name: formData.name,
            email: formData.email,
            phone: formData.phone,
            idNumber: formData.idNumber,
          },
          notes: formData.notes,
        }),
      });

      if (response.ok) {
        const data = await response.json();
        const booking = data.item || data;
        const bookingId = booking?.id;
        
        if (bookingId) {
          // ? ≈÷«›… «·ÕÃ“ ≈·Ï Context ›Ê—« ( ÕœÌÀ Ã„Ì⁄ «·’›Õ« !)
          addBooking(booking);
          
          // «· ÊÃÌÂ ≈·Ï ’›Õ… «·œ›⁄ „»«‘—… ?
          await router.push(`/booking/${bookingId}/payment`);
        } else {
          alert(' „ ≈‰‘«¡ «·ÕÃ“ ·ﬂ‰ ·„ ‰ „ﬂ‰ „‰ «·Õ’Ê· ⁄·Ï —ﬁ„ «·ÕÃ“');
          await router.push(`/properties/${propertyId}`);
        }
      } else {
        const errorData = await response.json();
        alert('›‘· ›Ì ≈‰‘«¡ «·ÕÃ“: ' + (errorData.error || 'Œÿ√ €Ì— „⁄—Ê›'));
      }
    } catch (error) {

      alert('ÕœÀ Œÿ√ √À‰«¡ «·ÕÃ“: ' + (error as Error).message);
    } finally {
      setSubmitting(false);
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-green-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Ã«—Ì «· Õ„Ì·...</p>
        </div>
      </div>
    );
  }

  if (!property) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">«·⁄ﬁ«— €Ì— „ÊÃÊœ</h2>
          <InstantLink 
            href="/properties"
            className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors inline-block"
          >
            «·⁄Êœ… ≈·Ï «·⁄ﬁ«—« 
          </InstantLink>
        </div>
      </div>
    );
  }

  const totalMonths = parseInt(formData.duration);
  const totalAmount = property.priceOMR * totalMonths;
  
  //  ÕÊÌ· «·»Ì«‰«  ≈·Ï ‰’ ¬„‰
  const safeTitle = toSafeText(property.title, 'ar');
  const safeType = toSafeText(property.type, 'ar');

  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <Head>
        <title>ÕÃ“ {safeTitle} | Ain Oman</title>
      </Head>

      <div className="max-w-4xl mx-auto px-4">
        {/* Header */}
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-3xl font-bold text-gray-900">ÕÃ“ «·⁄ﬁ«—</h1>
            <InstantLink
              href={`/properties/${propertyId}`}
              className="flex items-center gap-2 text-green-600 hover:text-green-700"
            >
              <FaArrowRight />
              «·⁄Êœ… ≈·Ï «·⁄ﬁ«—
            </InstantLink>
          </div>

          {/* Property Info */}
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
            {property.images && property.images[0] && (
              <InstantImage src={property.images[0]}
                alt={safeTitle}
                className="w-24 h-24 object-cover rounded-lg"
               loading="lazy" width={96} height={96}/>
            )}
            <div>
              <h2 className="text-xl font-semibold text-gray-900">{safeTitle}</h2>
              <p className="text-gray-600">{safeType}</p>
              <p className="text-2xl font-bold text-green-600 mt-2">
                {property.priceOMR} —Ì«· / ‘Â—
              </p>
            </div>
          </div>
        </div>

        {/* Progress Steps */}
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            {[
              { num: 1, label: '«·»Ì«‰«  «·‘Œ’Ì…' },
              { num: 2, label: ' ›«’Ì· «·ÕÃ“' },
              { num: 3, label: '«·„—«Ã⁄… Ê«· √ﬂÌœ' },
            ].map((s, i) => (
              <div key={s.num} className="flex items-center flex-1">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                    step >= s.num
                      ? 'bg-green-600 text-white'
                      : 'bg-gray-200 text-gray-600'
                  }`}
                >
                  {s.num}
                </div>
                <div className="ml-3 flex-1">
                  <p
                    className={`text-sm font-medium ${
                      step >= s.num ? 'text-green-600' : 'text-gray-600'
                    }`}
                  >
                    {s.label}
                  </p>
                </div>
                {i < 2 && (
                  <div
                    className={`h-1 w-full mx-4 ${
                      step > s.num ? 'bg-green-600' : 'bg-gray-200'
                    }`}
                  />
                )}
              </div>
            ))}
          </div>
        </div>

        {/* Form */}
        <div className="bg-white rounded-2xl shadow-lg p-6">
          {/* Step 1: Personal Info */}
          {step === 1 && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">«·»Ì«‰«  «·‘Œ’Ì…</h2>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <FaUser className="inline mr-2" />
                  «·«”„ «·ﬂ«„· *
                </label>
                <input
                  type="text"
                  required
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="√œŒ· «”„ﬂ «·ﬂ«„·"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <FaEnvelope className="inline mr-2" />
                  «·»—Ìœ «·≈·ﬂ —Ê‰Ì *
                </label>
                <input
                  type="email"
                  required
                  value={formData.email}
                  onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="email@example.com"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <FaPhone className="inline mr-2" />
                  —ﬁ„ «·Â« › *
                </label>
                <input
                  type="tel"
                  required
                  value={formData.phone}
                  onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="+968 9XXX XXXX"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  —ﬁ„ «·ÂÊÌ… / «·≈ﬁ«„… *
                </label>
                <input
                  type="text"
                  required
                  value={formData.idNumber}
                  onChange={(e) => setFormData({ ...formData, idNumber: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="—ﬁ„ «·ÂÊÌ…"
                />
              </div>

              <button
                onClick={() => setStep(2)}
                disabled={!formData.name || !formData.email || !formData.phone || !formData.idNumber}
                className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                «· «·Ì
                <FaArrowLeft />
              </button>
            </div>
          )}

          {/* Step 2: Booking Details */}
          {step === 2 && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6"> ›«’Ì· «·ÕÃ“</h2>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <FaCalendarAlt className="inline mr-2" />
                   «—ÌŒ «·»œ¡ *
                </label>
                <input
                  type="date"
                  required
                  value={formData.startDate}
                  onChange={(e) => setFormData({ ...formData, startDate: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  min={new Date().toISOString().split('T')[0]}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  „œ… «·≈ÌÃ«— (»«·√‘Â—) *
                </label>
                <select
                  value={formData.duration}
                  onChange={(e) => setFormData({ ...formData, duration: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="3">3 √‘Â—</option>
                  <option value="6">6 √‘Â—</option>
                  <option value="12">”‰… Ê«Õœ…</option>
                  <option value="24">”‰ «‰</option>
                  <option value="36">3 ”‰Ê« </option>
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <FaCreditCard className="inline mr-2" />
                  ÿ—Ìﬁ… «·œ›⁄ *
                </label>
                <select
                  value={formData.paymentMethod}
                  onChange={(e) => setFormData({ ...formData, paymentMethod: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="bank_transfer"> ÕÊÌ· »‰ﬂÌ</option>
                  <option value="card">»ÿ«ﬁ… «∆ „«‰</option>
                  <option value="cheque">‘Ìﬂ</option>
                  <option value="cash">‰ﬁœÌ</option>
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  „·«ÕŸ«  ≈÷«›Ì…
                </label>
                <textarea
                  value={formData.notes}
                  onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                  rows={4}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="√Ì „·«ÕŸ«  √Ê ÿ·»«  Œ«’…..."
                />
              </div>

              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <FaInfoCircle className="text-blue-600 mt-1" />
                  <div>
                    <p className="font-medium text-blue-900">„·Œ’ «· ﬂ·›…</p>
                    <p className="text-sm text-blue-700 mt-1">
                      «·≈ÌÃ«— «·‘Â—Ì: {property.priceOMR} —Ì«· ◊ {formData.duration} ‘Â—
                    </p>
                    <p className="text-lg font-bold text-blue-900 mt-2">
                      «·„Ã„Ê⁄: {totalAmount} —Ì«·
                    </p>
                  </div>
                </div>
              </div>

              <div className="flex gap-4">
                <button
                  onClick={() => setStep(1)}
                  className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-lg hover:bg-gray-300 transition-colors font-medium flex items-center justify-center gap-2"
                >
                  <FaArrowRight />
                  «·”«»ﬁ
                </button>
                <button
                  onClick={() => setStep(3)}
                  disabled={!formData.startDate}
                  className="flex-1 bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  «· «·Ì
                  <FaArrowLeft />
                </button>
              </div>
            </div>
          )}

          {/* Step 3: Review & Confirm */}
          {step === 3 && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">«·„—«Ã⁄… Ê«· √ﬂÌœ</h2>

              <div className="bg-gray-50 rounded-lg p-6 space-y-4">
                <h3 className="font-semibold text-gray-900 text-lg">»Ì«‰«  «·ÕÃ“</h3>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-600">«·«”„</p>
                    <p className="font-medium text-gray-900">{formData.name}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">«·»—Ìœ «·≈·ﬂ —Ê‰Ì</p>
                    <p className="font-medium text-gray-900">{formData.email}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">—ﬁ„ «·Â« ›</p>
                    <p className="font-medium text-gray-900">{formData.phone}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">—ﬁ„ «·ÂÊÌ…</p>
                    <p className="font-medium text-gray-900">{formData.idNumber}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600"> «—ÌŒ «·»œ¡</p>
                    <p className="font-medium text-gray-900">{formData.startDate}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">«·„œ…</p>
                    <p className="font-medium text-gray-900">{formData.duration} ‘Â—</p>
                  </div>
                </div>
              </div>

              <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                <h3 className="font-semibold text-green-900 text-lg mb-4">„·Œ’ «·›« Ê—…</h3>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-gray-700">«·≈ÌÃ«— «·‘Â—Ì</span>
                    <span className="font-medium">{property.priceOMR} —Ì«·</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-700">«·„œ…</span>
                    <span className="font-medium">{formData.duration} ‘Â—</span>
                  </div>
                  <div className="border-t border-green-300 pt-2 mt-2 flex justify-between">
                    <span className="font-bold text-gray-900">«·„Ã„Ê⁄ «·ﬂ·Ì</span>
                    <span className="font-bold text-green-600 text-xl">{totalAmount} —Ì«·</span>
                  </div>
                </div>
              </div>

              <div className="flex items-start gap-3 p-4 border border-gray-300 rounded-lg">
                <input
                  type="checkbox"
                  id="terms"
                  checked={formData.agreedToTerms}
                  onChange={(e) => setFormData({ ...formData, agreedToTerms: e.target.checked })}
                  className="mt-1"
                />
                <label htmlFor="terms" className="text-sm text-gray-700">
                  √Ê«›ﬁ ⁄·Ï{' '}
                  <InstantLink href="/terms" className="text-green-600 hover:underline">
                    «·‘—Êÿ Ê«·√Õﬂ«„
                  </InstantLink>
                  {' '}Ê{' '}
                  <InstantLink href="/privacy" className="text-green-600 hover:underline">
                    ”Ì«”… «·Œ’Ê’Ì…
                  </InstantLink>
                </label>
              </div>

              <div className="flex gap-4">
                <button
                  onClick={() => setStep(2)}
                  className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-lg hover:bg-gray-300 transition-colors font-medium flex items-center justify-center gap-2"
                >
                  <FaArrowRight />
                  «·”«»ﬁ
                </button>
                <button
                  onClick={handleSubmit}
                  disabled={!formData.agreedToTerms || submitting}
                  className="flex-1 bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    <>
                      <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />
                      Ã«—Ì «·ÕÃ“...
                    </>
                  ) : (
                    <>
                      <FaCheckCircle />
                       √ﬂÌœ «·ÕÃ“ Ê«·«‰ ﬁ«· ··œ›⁄
                    </>
                  )}
                </button>
              </div>
            </div>
          )}
        </div>

        {/* Help Section */}
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
          <p className="text-blue-900 font-medium mb-2"> Õ «Ã „”«⁄œ…ø</p>
          <p className="text-sm text-blue-700 mb-4">
            ›—Ìﬁ‰« „ «Õ ⁄·Ï „œ«— «·”«⁄… ·„”«⁄œ ﬂ ›Ì ≈ﬂ„«· «·ÕÃ“
          </p>
          <div className="flex gap-3 justify-center">
            <InstantLink
              href="/contact"
              className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors"
            >
               Ê«’· „⁄‰«
            </InstantLink>
            <a
              href={`https://wa.me/96800000000?text=${encodeURIComponent('√Õ «Ã „”«⁄œ… ›Ì «·ÕÃ“')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 transition-colors flex items-center gap-2"
            >
              <FaPhone />
              « ’· »‰«
            </a>
          </div>
        </div>
      </div>
    </div>
  );
}


