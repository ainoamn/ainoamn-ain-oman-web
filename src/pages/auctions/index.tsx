...
export default function AuctionsPage() {
  ...
  useEffect(() => {
    ...
    const mock: Auction[] = [
      { 
        ...
        bedrooms: 5, 
        bathrooms: 4, 
        startTime: now + 24*3600*1000, // Ì»œ√ »⁄œ ÌÊ„
        endTime: now + 72*3600*1000,   // Ì‰ ÂÌ »⁄œ 3 √Ì«„
      ...
      { 
        ...
        bedrooms: 3, 
        bathrooms: 2, 
        startTime: now - 24*3600*1000, // »œ√ „‰– ÌÊ„
        endTime: now + 24*3600*1000,   // Ì‰ ÂÌ »⁄œ ÌÊ„
      ...
      { 
        id: "auction4", 
        title: "‘ﬁ… œÊ»·ﬂ” ›«Œ—…", 
        description: "œÊ»·ﬂ” 220 „≤ »≈ÿ·«·… »«‰Ê—«„Ì….", 
        location: "„”ﬁÿ° „œÌ‰… «·”·ÿ«‰ ﬁ«»Ê”", 
        price: 160000, 
        currentBid: 130000, 
        area: 220, 
        bedrooms: 4, 
        bathrooms: 3, 
        startTime: now + 48*3600*1000, // Ì»œ√ »⁄œ ÌÊ„Ì‰
        endTime: now + 120*3600*1000,  // Ì‰ ÂÌ »⁄œ 5 √Ì«„
      ...
      { 
        ...
        bedrooms: 8, 
        bathrooms: 6, 
        startTime: now - 120*3600*1000, // »œ√ „‰– 5 √Ì«„
        endTime: now - 24*3600*1000,    // «‰ ÂÏ „‰– ÌÊ„
      ...
      { 
        ...
        bedrooms: 6, 
        bathrooms: 5, 
        startTime: now - 96*3600*1000, // »œ√ „‰– 4 √Ì«„
        endTime: now - 48*3600*1000,   // «‰ ÂÏ „‰– ÌÊ„Ì‰
      ...
      { 
        ...
        bedrooms: 2, 
        bathrooms: 2, 
        startTime: now - 12*3600*1000, // »œ√ „‰– 12 ”«⁄…
        endTime: now + 36*3600*1000,   // Ì‰ ÂÌ »⁄œ ÌÊ„ Ê‰’›
      ...
      { 
        ...
        bedrooms: 4, 
        bathrooms: 3, 
        startTime: now + 72*3600*1000, // Ì»œ√ »⁄œ 3 √Ì«„
        endTime: now + 240*3600*1000,  // Ì‰ ÂÌ »⁄œ 10 √Ì«„
  ...
  const formatPrice = (n: number) => new Intl.NumberFormat("ar-OM").format(n) + " —.⁄";
  ...
  const getTimeRemaining = (endTime: number) => {
    ...
    return `${days} ÌÊ„ ${hours} ”«⁄… ${minutes} œﬁÌﬁ…`;
  };

  const getStatusBadge = (status: string, featured?: boolean) => {
    if (status === 'ended') return "„‰ ÂÌ";
    if (status === 'upcoming') return "ﬁ«œ„";
    return featured ? "„„Ì“" : "‰‘ÿ";
  };
          ...
          {showFilters && (
            <div className={`mb-8 p-6 rounded-2xl shadow-lg ${isDark ? "bg-gray-800" : "bg-white"}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-2">‰Ê⁄ «·⁄ﬁ«—</label>
                  <select 
                    className="w-full p-3 rounded-lg border"
                    value={filterOptions.propertyType}
                    onChange={(e) => setFilterOptions({...filterOptions, propertyType: e.target.value})}
                  >
                    <option value="all">Ã„Ì⁄ «·√‰Ê«⁄</option>
                    <option value="villas">›Ì·« </option>
                    <option value="apartments">‘ﬁﬁ</option>
                    <option value="lands">√—«÷Ì</option>
                  </select>
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-2">«·„Õ«›Ÿ…</label>
                  <select 
                    className="w-full p-3 rounded-lg border"
                    value={filterOptions.location}
                    onChange={(e) => setFilterOptions({...filterOptions, location: e.target.value})}
                  >
                    <option value="all">Ã„Ì⁄ «·„Õ«›Ÿ« </option>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Õ«·… «·„“«œ</label>
                  <select 
                    className="w-full p-3 rounded-lg border"
                  ...
                  >
                    ...
                    <option value="upcoming">ﬁ«œ„…</option>
                    <option value="ended">„‰ ÂÌ…</option>
                  </select>
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-2">«· — Ì» Õ”»</label>
                  <select 
                    className="w-full p-3 rounded-lg border"
                    value={filterOptions.sortBy}
                    onChange={(e) => setFilterOptions({...filterOptions, sortBy: e.target.value})}
                  >
                    <option value="featured">«·„„Ì“… √Ê·«</option>
                    <option value="price-low">«·”⁄— („‰Œ›÷ ≈·Ï „— ›⁄)</option>
                    <option value="price-high">«·”⁄— („— ›⁄ ≈·Ï „‰Œ›÷)</option>
                    <option value="ending"> ‰ ÂÌ ﬁ—Ì»«</option>
                  </select>
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-2">√ﬁ· ”⁄— (—.⁄)</label>
                  <input
                    type="number"
                    className="w-full p-3 rounded-lg border"
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">√⁄·Ï ”⁄— (—.⁄)</label>
                  <input
                    type="number"
                    className="w-full p-3 rounded-lg border"
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">√ﬁ· ⁄œœ €—›</label>
                  <input
                    type="number"
                    className="w-full p-3 rounded-lg border"
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">√ﬁ· „”«Õ… („≤)</label>
                  <input
                    type="number"
                    className="w-full p-3 rounded-lg border"
          ...
          {loading ? (
            <div className={`min-h-[200px] flex items-center justify-center ${isDark ? "text-white" : "text-gray-800"}`}>
              <div className="text-center">
                ...
                <p className="text-xl">Ã«—Ì  Õ„Ì· «·⁄ﬁ«—« ...</p>
              </div>
            </div>
          ) : (
            <>
              ...
              {filteredFeaturedActive.length > 0 && (
                <section className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>«·„“«œ«  «·„„Ì“… «·‰‘ÿ…</h2>
                            ...
                            <div className="absolute bottom-4 left-4 right-4">
                              <div className={`p-3 rounded-lg backdrop-blur-sm ${isDark ? "bg-black/30 text-white" : "bg-white/90 text-gray-800"}`}>
                                <div className="flex justify-between items-center">
                                  <div className="font-medium">«·Êﬁ  «·„ »ﬁÌ</div>
                                  <div className="flex gap-3 text-sm">
                                    <span className="text-center"><b>{days}</b> ÌÊ„</span>
                                    <span className="text-center"><b>{hours}</b> ”«⁄…</span>
                                    <span className="text-center"><b>{minutes}</b> œﬁÌﬁ…</span>
                                  </div>
                                </div>
                              </div>
                            </div>
              ...
              {filteredActive.length > 0 && (
                <section className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>«·„“«œ«  «·‰‘ÿ…</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {filteredUpcoming.length > 0 && (
                <section className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>«·„“«œ«  «·ﬁ«œ„…</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {filteredFeaturedEnded.length > 0 && (
                <section className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>«·„“«œ«  «·„‰ ÂÌ… «·„„Ì“…</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        <div className="p-6">
                          ...
                          <div className={`p-4 rounded-lg mb-4 ${isDark ? "bg-gray-700" : "bg-gray-100"}`}>
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <div>
                                <div className="text-xs text-gray-500">«·”⁄— «·‰Â«∆Ì</div>
                                <div className={isDark ? "text-green-300 font-semibold" : "text-green-600 font-semibold"}>{formatPrice(p.currentBid)}</div>
                              </div>
                            </div>
                          </div>
              ...
              {filteredEnded.length > 0 && (
                <section className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>«·„“«œ«  «·„‰ ÂÌ…</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredEnded.map((p) => (
                      <article key={p.id} className={`rounded-xl overflow-hidden shadow-xl ${isDark ? "bg-gray-800" : "bg-white"}`}>
                        <div className="relative">
              ...
              {/* Œ—Ìÿ… «·„Ê«ﬁ⁄ */}
              <section className="mb-12">
                <h2 className={`text-2xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-800"}`}>„Ê«ﬁ⁄ «·„“«Ìœ« </h2>
                <div className="rounded-xl overflow-hidden shadow-xl">
                  {!hasMapsKey ? (
                    <div className={`p-12 text-center ${isDark ? "bg-gray-800 text-gray-200" : "bg-white text-gray-600"}`}>
                      ·„ Ì „ ÷»ÿ „› «Õ Google Maps (NEXT_PUBLIC_GOOGLE_MAPS_API_KEY)
                    </div>
                  ) : mapError ? (
