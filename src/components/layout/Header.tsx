...
import { getT, norm, isRTL } from "@/lib/i18n";
...
export default function Header() {
  ...
  const dir: "rtl" | "ltr" = isRTL(router.locale || "ar") ? "rtl" : "ltr";
  ...
  const [langs, setLangs] = useState<string[]>(["ar", "en", "fr", "hi", "fa", "ur"]);
  useEffect(() => {
    fetch("/api/i18n")
      .then(r => r.json())
      .then(d => Array.isArray(d?.langs) && d.langs.length && setLangs(d.langs))
      .catch(() => {});
  }, []);
  ...
  const labelOf = (code: string) =>
    ({ ar: "العربية", en: "English", fr: "Français", hi: "हिन्दी", fa: "فارسی", ur: "اردو" } as Record<string, string>)[code] || code.toUpperCase();

  return (
    <div dir={dir} className={`sticky top-0 z-50 transition-transform duration-300 ${hiddenOnScroll ? "-translate-y-full" : "translate-y-0"}`}>
        ...
        <div className="mx-auto max-w-7xl px-3 sm:px-4 lg:px-6">
          <div className="flex items-center justify-between py-3">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {links.map((l) => (
                <Link key={l.href} href={l.href} className="px-3 py-2 rounded-xl font-medium text-white/90 hover:text-white transition-all duration-200 hover:shadow hover:shadow-black/10 hover:ring-2 hover:ring-white/30">
                  {l.label}
                </Link>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-2">
              <select
                ...
                className="text-sm rounded-xl border border-white/25 bg-white/10 text-white/95 px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-white/50"
                aria-label="Change language"
              >
                {langs.map((l) => (
                  <option key={l} value={l}>{labelOf(l)}</option>
                ))}
              </select>
          ...
          <div className="lg:hidden border-t border-white/20 bg-white text-gray-800">
            <div className="mx-auto max-w-7xl px-3 sm:px-4 lg:px-6 py-3 space-y-1">
              ...
              <details className="group">
                <summary className="px-3 py-2 rounded-xl cursor-pointer hover:bg-teال-50 hover:text-teal-700">{tt("nav.more")}</summary>
