...
export function AI18nProvider({ children }: { children: React.ReactNode }) {
  ...
  const t = useMemo(() => {
    return (text: string, vars?: Record<string, string | number>) => {
      if (lang === "ar") return formatVars(text, vars);
      const k = keyOf(text, lang);
...
export function Trans({ children, vars }: { children: string; vars?: Record<string, string | number> }) {
  const { t } = useAI18n();
